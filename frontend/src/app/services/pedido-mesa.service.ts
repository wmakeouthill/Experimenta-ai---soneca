...
export class PedidoMesaService {
   ...
   */
  criarPedido(
    request: CriarPedidoMesaRequest,
    idempotencyKey: string
  ): Observable<PedidoMesaResponse> {
  ...
  buscarStatusPedido(pedidoId: string): Observable<StatusPedidoCliente> {
    ...
    return this.http.get<StatusPedidoCliente>(
      `${this.publicApiUrl}/pedido/${pedidoId}/status?${noCache}`
    );
  }
  ...
  buscarHistoricoPedidos(
    ...
    pagina: number,
    tamanho: number
  ): Observable<HistoricoPedidosResponse> {
  ...
  salvarSenhaCliente(clienteId: string, novaSenha: string, senhaAtual?: string): Observable<void> {
    return this.http.put<void>(`/api/cliente/conta/senha`, { novaSenha, senhaAtual });
  }
  ...
  avaliarProduto(
    ...
    return this.http.post<void>(`/api/cliente/conta/avaliacoes`, {
      produtoId,
      pedidoId,
      nota,
      comentario,
   ...
   * Headers são adicionados automaticamente pelo clienteAuthInterceptor
   */
  buscarAvaliacoesCliente(
    clienteId: string
  ): Observable<{ produtoId: string; pedidoId: string; nota: number; comentario?: string }[]> {
    return this.http.get<
      { produtoId: string; pedidoId: string; nota: number; comentario?: string }[]
