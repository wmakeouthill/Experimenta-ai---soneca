...
import {
  ...
  ChatIAResponse,
  ChatIAService,
  ConversaSalva as ConversaSalvaAPI,
  ProdutoDestacado,
  SalvarConversaDTO,
} from '../../../services/chat-ia.service';
...
export { AcaoChat, ProdutoDestacado, TipoAcao } from '../../../services/chat-ia.service';
    ...
    return parsed.map(
      (
        c: ConversaSalva & {
          ...
          mensagens: Array<MensagemChat & { timestamp: string }>;
        }
      ) => ({
        ...c,
...
export function useChatIA(
  ...
  function salvarConversaAtualNoHistorico(): void {
    ...
    const conversaId = conversaHistoricoAtualId || gerarUuid();
    ...
    const conversaAtual: ConversaSalva = {
      ...
      previewUltimaMensagem:
        msgs.at(-1)!.text.substring(0, 50) + (msgs.at(-1)!.text.length > 50 ? '...' : ''),
    };
      ...
      chatService.salvarConversa(clienteId, dto).subscribe({
        next: conversaSalva => {
          if (conversaSalva) {
            ...
            const historico = [
              novaConversa,
              ...historicoConversas().filter(c => c.id !== conversaSalva.id),
        ...
        error: () => {
          ...
          const historico = [
            conversaAtual,
            ...historicoConversas().filter(c => c.id !== conversaId),
          ].slice(0, MAX_CONVERSAS_HISTORICO);
          historicoConversas.set(historico);
