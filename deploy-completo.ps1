...
if ($deploy -eq "S" -or $deploy -eq "s" -or $deploy -eq "Y" -or $deploy -eq "y") {
    ...
    gcloud run deploy experimenta-ai-soneca `
        ...
        --set-env-vars="^@^DB_URL=$DB_URL@DB_USERNAME=root@SERVER_PORT=8080@SPRING_PROFILES_ACTIVE=prod,cloud@SHOW_SQL=false@JWT_EXPIRATION=86400@LOG_LEVEL=INFO@OPENAI_MODEL=gpt-4o-mini@OPENAI_MAX_TOKENS=4000@OPENAI_MODELS_FALLBACK=gpt-4o-mini,gpt-3.5-turbo" `
        --project=$PROJECT_ID
